...
class Database {
     ...
     * @return MysqliDb
     */
    public function join($joinTable, $joinCondition, $joinType = '', array $conditions = array()) {
        ...
        $this->_join[] = Array($joinType, $joinTable, $joinCondition, $conditions);
    ...
    public function buildJoinConditions(array $conditions) {
        /**
         * Aby oszukać warunek count($this->_where) w dodawaniu łącznika
         */
        $this->_where[] = 'JOINING CONDITIONS';

        foreach ($conditions as $arguments) {
            call_user_func_array(array($this, $arguments[0]), array_slice($arguments, 1));
        }

        $this->_buildConditionCounter = 1;
        $this->_buildCondition('', array_slice($this->_where, 1));

        return $this->replacePlaceHolders($this->_query, $this->_bindParams);
    }
    ...
    protected function _buildJoin() {
        ...
        foreach ($this->_join as $data) {
            list ($joinType, $joinTable, $joinCondition, $conditions) = $data;
            ...
            if (count($conditions)) {
                $this->_query .= ' ' . $this->subQuery()->buildJoinConditions($conditions);
            }
        }
    }
    ...
    private $_buildConditionCounter = 0;
    ...
    protected function _buildCondition($operator, &$conditions) {
        ...
        foreach ($conditions as $cond) {
            ...
            if ($this->_buildConditionCounter++ != 0) {
                $this->_query .= " $concat ";
            }
    ...
    protected function _prepareQuery() {
        if (!$stmt = $this->mysqli()->prepare($this->_query)) {
            ...
            echo CHtml::tag('div', array('class' => 'alert alert-danger'), $this->_lastQuery);
...
use PioCMS\Traits\Singleton;

// pobrane z http://culttt.com/2012/10/01/roll-your-own-pdo-php-class/
class Database {
    ...
    public function __construct() {
        ...
        try {
            $this->_connection = new \PDO('mysql:host=' . DATABASE_HOST . ';dbname=' . DATABASE_NAME . ';charset=utf8', DATABASE_USER, DATABASE_PASSOWD, $db_options);
        } catch (\Exception $e) {
            echo $e->getMessage();
            exit;
        }
